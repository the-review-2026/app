export default {
  async fetch(request, env) {
    ...
    if (pathname === "/manager/stats" && request.method === "GET") {
      return getManagerStats(request, env);
    }
...
// Temporary: allow authenticated non-members to exercise Manager features during UI verification.
const TEMPORARY_MANAGER_PREVIEW_ACCESS = true;
...
async function getManagerStats(request, env) {
  const access = await requireManagerOwner(request, env);
  if (!access.ok) {
    return json(access.body, access.status);
  }

  const [usersResponse, flashcardsResponse, databaseSizeResult] = await Promise.all([
    supabaseRequest(access.supabase, `/users?select=${USER_SELECT_FIELDS}&order=created_at.desc`),
    supabaseRequest(access.supabase, `/flashcard?select=${FLASHCARD_SELECT_FIELDS}`),
    getSupabaseDatabaseSize(access.supabase, env),
  ]);

  if (!usersResponse.ok) {
    const error = await supabaseError(usersResponse, "Failed to read dashboard user statistics");
    return json(error.body, error.status);
  }

  const usersText = await usersResponse.text();
  const flashcardsText = flashcardsResponse.ok ? await flashcardsResponse.text() : "[]";
  let users = [];
  try {
  ...
  const estimatedBytes =
    new TextEncoder().encode(usersText).byteLength +
    new TextEncoder().encode(flashcardsText).byteLength;
  const databaseBytes =
    Number.isFinite(databaseSizeResult.bytes) && databaseSizeResult.bytes >= 0
      ? databaseSizeResult.bytes
      : estimatedBytes;

  return json({
    userCount: users.length,
    memberCount: users.filter((user) => Boolean(normalizeManagerRole(user?.manager_role))).length,
    databaseBytes,
    databaseSizeSource: databaseSizeResult.source,
  });
}

async function getSupabaseDatabaseSize(supabase, env) {
  const configuredBytes = Number(env?.SUPABASE_DATABASE_SIZE_BYTES);
  if (Number.isFinite(configuredBytes) && configuredBytes >= 0) {
    return { bytes: configuredBytes, source: "database" };
  }

  try {
    const response = await supabaseRequest(supabase, "/rpc/get_database_size", {
      method: "POST",
      body: "{}",
    });
    if (response.ok) {
      const bytes = readDatabaseSizeBytes(await response.json());
      if (Number.isFinite(bytes) && bytes >= 0) {
        return { bytes, source: "database" };
      }
    }
  } catch {
    // The optional RPC is not installed; the dashboard will use a payload estimate.
  }

  return { bytes: null, source: "estimate" };
}

function readDatabaseSizeBytes(value) {
  if (typeof value === "number" || typeof value === "string") {
    const bytes = Number(value);
    return Number.isFinite(bytes) ? bytes : null;
  }
  if (Array.isArray(value)) {
    return value.length > 0 ? readDatabaseSizeBytes(value[0]) : null;
  }
  if (value && typeof value === "object") {
    for (const key of ["database_bytes", "database_size", "size_bytes", "bytes", "size"]) {
      if (Object.prototype.hasOwnProperty.call(value, key)) {
        const bytes = readDatabaseSizeBytes(value[key]);
...
async function requireManagerOwner(request, env) {
  ...
  if (memberResult.member?.role !== "owner" && !TEMPORARY_MANAGER_PREVIEW_ACCESS) {
    return {
      ok: false,
      status: 403,
      body: {
...
async function requireManagerPermission(request, env, permissions = []) {
  ...
  const allowed =
    TEMPORARY_MANAGER_PREVIEW_ACCESS ||
    role === "owner" ||
    permissions.some((permission) => rolePermissions[permission]);
  if (!allowed) {
    return {
      ok: false,
      status: 403,
...
async function listFlashcards(request, env) {
  const supabase = getSupabaseConfig(env);
  if (!supabase) {
    return json(getFallbackFlashcards());
  }
